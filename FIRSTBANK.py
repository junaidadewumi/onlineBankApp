...
def FBN():
    print(f"Welcome to First Bank of Nigeria (FBN Plc).")
    time.sleep(2)
    print(f""""
...
def Registration():
    ...
    acc_info = ("First_name", "middle_name", "Last_name", "Email", "BVN", "Account_Number", "'Pswd", "Confirm_pswd",
        "Address")
    querry = ("""INSERT INTO FBN(First_name, Middle_name, Last_name, Email, BVN, Account_Number, Pswd, Confirm_pswd,
...
def log_in():
    ...
    querry = "select * from FBN where Email = %s and Pswd = %s"
    cursor.execute(querry, val)
...
def Transfer_gtb():
    ...
    if result:
        ...
        querry = "update fbn set Balance = %s where Email = %s and Pswd = %s"
        cursor.execute(querry, val)
        myconn.commit()
        fbnbal = result[10] + amount
        val2 = (fbnbal, trf)
        querry2 = "update gtbbank set Balance = %s where Account_Number = %s"
...
def Airtime_purchase():
    ...
    querry = "select * from fbn where Email = %s"
    val = (username, )
    cursor.execute (querry, val)
    result = cursor.fetchone()
    newbal = result[10] - amount
    new_querry = "UPDATE fbn SET Balance = %s WHERE Pswd = %s"
